...
filelist = os.listdir("processed_train")

# function that processes the training sets
def process_train(filelist,batch_index):
	data = []
	targets = []
	for i in xrange (25001):
		filename = filelist[i]
		names = filename.split(".")
		if not names[-1] == "jpg":
			continue

		if not int(names[1].split("_processed")[0]) % 125 == batch_index:
		 	continue
		...
		img = Image.open(open("/".join(["processed_train",filename])))
...
for batch_index in xrange(1,126,1):
	data,targets = process_train(filelist,batch_index)
	data = numpy.array(data)
	targets = numpy.array(targets)
	train_set = (data,targets)
	pickle.dump(train_set,open(str(("_").join([str(batch_index),"dataset.p"])),'wb'))
